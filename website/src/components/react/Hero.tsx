...
1. Create a \`fastci.config.json\` file in the repository root with:
...
2. Scan all workflow files in \`.github/workflows/\` for FastCI usage:

   For each workflow using \`jfrog-fastci/fastci@v0\`:
...
3. Add FastCI as the first step in each job (before actions/checkout). No \`with:\` block is needed - the token is provided automatically:

...
async function traceWorkflowRun(
  ...
  const now = new Date();
  await sendTraceWorkflowRunLog(processTrees, workflowRun, jobs, jobAnnotations, prLabels);
  ...
  return await tracer.startActiveSpan(
    workflowRun.name ?? (workflowRun as any).display_title,
...
function workflowRunToAttributes(
  ...
  return {
    ...
    "github.display_title": (workflowRun as any).display_title,
